...
const headCells = [
  ...
  { id: 'contactPerson', numeric: false, disablePadding: false, label: 'Contact' },
  { id: 'projectsInvolved', numeric: true, disablePadding: false, label: 'Projects' },
...
export default function RoadSafetyActorsPage() {
  ...
  const sortedAndPagedActors = useMemo(() => {
    const sorted = [...filteredActors].sort((a, b) => {
        ...
        if (orderBy === 'projectsInvolved') {
            return isAsc ? a.projectsInvolved - b.projectsInvolved : b.projectsInvolved - a.projectsInvolved;
        }
        return isAsc
  ...
  const actorTypes = useMemo(() => [...new Set(mockActors.map(a => a.type))].length, []);

  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4" fontWeight={600}>Road Safety Actors</Typography>
        <Button variant="contained" startIcon={<AddIcon />}>Add New Actor</Button>
      </Box>

      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={4}>
            <Card><CardContent><Typography variant="h6">Total Actors</Typography><Typography variant="h4">{mockActors.length}</Typography></CardContent></Card>
        </Grid>
        <Grid item xs={12} sm={4}>
            <Card><CardContent><Typography variant="h6">Actor Types</Typography><Typography variant="h4">{actorTypes}</Typography></CardContent></Card>
        </Grid>
        <Grid item xs={12} sm={4}>
            <Card><CardContent><Typography variant="h6">Active Engagements</Typography><Typography variant="h4">{mockActors.filter(a => a.status === 'active').length}</Typography></CardContent></Card>
        </Grid>
      </Grid>
                ...
                <TableRow hover key={actor.id}>
                  <TableCell component="th" scope="row" padding="none">
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Avatar sx={{ mr: 2, bgcolor: 'transparent' }}>{actor.icon}</Avatar>
                      <Box>
                        <Typography variant="subtitle2">{actor.name}</Typography>
                        <Typography variant="body2" color="text.secondary">ID: {actor.id}</Typography>
                      </Box>
                    </Box>
                  </TableCell>
                  ...
                  <TableCell>
                    <Box>
                        <Typography variant="body2">{actor.contactPerson}</Typography>
                        <Typography variant="caption" color="text.secondary">{actor.email}</Typography>
                    </Box>
                  </TableCell>
      ...
      >
        ...
        <MenuItem onClick={handleMenuClose} sx={{ color: 'error.main' }}>Remove Actor</MenuItem>
      </Menu>
    </Box>
  );
}
