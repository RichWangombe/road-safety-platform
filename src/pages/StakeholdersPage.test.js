...
describe("StakeholdersPage", () => {
  ...
  it("sorts stakeholders by name", async () => {
    ...
    const nameHeader = await within(table).findByRole("columnheader", {
      name: /Stakeholder/i,
    });
    ...
    await waitFor(() => {
      expect(
        within(updatedRows[1]).getByText("API Jane Smith"),
      ).toBeInTheDocument();
    });
    await waitFor(() => {
      expect(
        within(updatedRows[2]).getByText("API John Doe"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("displays correct counts in summary cards from API data", async () => {
    ...
    await waitFor(async () => {
      const totalStakeholdersCard = (
        await screen.findByText(/Total Stakeholders/i)
    ...
    await waitFor(async () => {
      const highEngagementCard = (
        await screen.findByText(/High-Engagement/i)
      ).closest(".MuiCard-root");
