...
export default function TeamMembersPage() {
  ...
  const sortedAndPagedMembers = useMemo(() => {
    const sorted = [...filteredMembers].sort((a, b) => {
        ...
        if (orderBy === 'assignedTasks') {
            return isAsc ? a.assignedTasks - b.assignedTasks : b.assignedTasks - a.assignedTasks;
        }
        return isAsc
  ...
  const activeMembers = useMemo(() => mockTeamMembers.filter(m => m.status === 'active').length, []);
  const pendingInvites = useMemo(() => mockTeamMembers.filter(m => m.status === 'pending').length, []);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={4}>
            <Card><CardContent><Typography variant="h6">Total Members</Typography><Typography variant="h4">{mockTeamMembers.length}</Typography></CardContent></Card>
        </Grid>
        <Grid item xs={12} sm={4}>
            <Card><CardContent><Typography variant="h6">Active Members</Typography><Typography variant="h4">{activeMembers}</Typography></CardContent></Card>
        </Grid>
        <Grid item xs={12} sm={4}>
            <Card><CardContent><Typography variant="h6">Pending Invitations</Typography><Typography variant="h4">{pendingInvites}</Typography></CardContent></Card>
        </Grid>
      </Grid>
                ...
                <TableRow hover key={member.id}>
                  <TableCell component="th" scope="row" padding="none">
                    <Box sx={{ display: 'flex', alignItems: 'center' }}>
                      <Avatar sx={{ mr: 2, bgcolor: 'secondary.main' }}>{member.avatar}</Avatar>
                      <Box>
      ...
      >
        ...
        <MenuItem onClick={handleMenuClose} sx={{ color: 'error.main' }}>Deactivate User</MenuItem>
      </Menu>
    </Box>
  );
}
