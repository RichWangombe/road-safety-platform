...
const TasksTable = ({ activityId }) => {
  const { user } = useAuth();
  const canManageTasks = user?.role === 'Program Manager' || user?.role === 'Team Lead';
  ...
  const handleFormSave = (taskData) => {
    if (editingTask) {
      ...
      const newTask = { ...taskData, id: Date.now(), activityId: parseInt(activityId) };
            ...
            {filteredTasks.length > 0 ? filteredTasks.map((task) => {
              ...
              return (
                <TableRow key={task.id} hover>
                  <TableCell>{task.name}</TableCell>
                  <TableCell>
                    <Chip label={task.status} size="small" sx={{ backgroundColor: statusColors.bg, color: statusColors.color, textTransform: 'capitalize' }} />
                  </TableCell>
      ...
      >
        {canManageTasks && [
          <MenuItem key="edit" onClick={() => {
            ...
            setIsFormOpen(true);
            handleMenuClose();
          }}><Edit fontSize="small" sx={{ mr: 1 }} />Edit Task</MenuItem>,
          <MenuItem key="delete" onClick={() => {
            setTaskToDelete(selectedTask);
            setIsConfirmOpen(true);
            handleMenuClose();
          }}><Delete fontSize="small" sx={{ mr: 1 }} />Delete Task</MenuItem>
        ]}
      </Menu>
